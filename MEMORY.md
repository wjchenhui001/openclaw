...
- 语言偏好:中文
- 首次对话日期:2026-04-04
- 主要沟通渠道:Feishu (direct message)
- 重视系统安全和自动化
- 偏好简洁直接的指令风格
- 已配置 Tavily API 密钥用于网络搜索
- Agents 已精简至 10 个核心 agents
...
1. **备份后未主动通知** — 04:00/12:00 备份完成但没发消息，用户主动问"你做了吗"才回应。教训：关键操作完成后必须在 heartbeat 中主动上报。
2. **未提前检查环境兼容性** — 尝试安装 ClawTeam 时，应先检查 Python 版本再动手。教训：动手前 `python3 --version` / `which cmd` 等预检查不可省。
3. **记忆文件重复写入** — `memory/2026-04-05.md` 被写入了 4 遍相同内容（~8KB）。教训：写入前先读、确认无重复再追加。
...
- **工具系统标准化** (2026-04-05)
  - 20 个标准化工具，`@tool` 装饰器 + `ToolRegistry` + JSON Schema
  - `ToolExecutor` 异步批量执行、错误分类重试
  - MCP Bridge 部署：13 个工具注册到 `openclaw.json` 的 `mcp.servers`

- **系统升级** (2026-04-05)
  - Python 升级到 3.14.3（用于技能依赖）
  - 模型配置：`openrouter/qwen/qwen3.6-plus:free`（当前会话实际使用）

- **用户核心偏好（必须记住）**
  - 主动通知：关键操作完成后必须告知用户
  - 成本敏感：优先使用免费模型
  - 简洁指令：单个动词立即执行
  - "维斯模式"：主动提醒、总结、优化（heartbeat 驱动）
