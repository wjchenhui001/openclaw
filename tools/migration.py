...
def web_search(query: str, count: int = 10):
    # 延迟导入，避免循环依赖
    try:
        # 尝试导入实际的 web_search 技能
        sys.path.insert(0, str(Path.home() / '.openclaw' / 'skills' / 'brave-web-search'))
        from brave_web_search import web_search as search_func
        ...
        return {
            ...
            "count": 0,
            "results": [],
            "note": f"Web search skill not available: {e}"
        }
